...
app.get('/add', function(req,res){
  db.serialize(()=>{
    ...
    [1 ,req.query.tarea, req.query.prioridad,  "-"], function(err) {
      if (err) {
        return console.log(err.message);
      }
      console.log("ingreso ok !");
      res.sendFile(path.join(__dirname,'./public/index.html'));
      db.run('update tareas set id=x.rn from (select tarea,row_number() over (partition by 1 order by prioridad desc) rn from tareas) x where tareas.tarea=x.tarea   ')
    });
  });
});
...
app.get('/view.json',function(req,res){
  db.serialize(()=>{
    db.all('select * from tareas order by prioridad desc', [], function(err,row) {
      if (err) {
        return console.log(err.message);
      }
      console.log(row);
